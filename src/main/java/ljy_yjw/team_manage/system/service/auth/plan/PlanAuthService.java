...
public class PlanAuthService {
	...
	@Memo("해당 일정에 대한 권한이 있는가를 체크하는 메소드 만약 있다면 해당 일정 리턴")
	public Team checkAuthThenGet(long seq, Users user) throws PlanByUserNotAuthException {
		PlanByUser planByUser = planByUserAPI.findBySeq(seq);
		if (planByUser == null) {
			throw new PlanByUserNotAuthException("해당 팀이 존재하지 않거나 자신의 일정이 아닙니다.");
		}
		return planByUser.getTeam();
	}
